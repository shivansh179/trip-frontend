...
export async function GET(req: NextRequest) {
  ...
  const col = db().collection('flight_bookings');
...
export async function POST(req: NextRequest) {
  try {
    ...
    const docId = booking.txnid || `flt-${Date.now()}`;
    await db().collection('flight_bookings').doc(docId).set({ ...booking, savedAt: new Date().toISOString() }, { merge: true });
